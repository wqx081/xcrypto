...
string StrCat(const AlphaNum &a, const AlphaNum &b) {
  string result;
  base::STLStringResizeUninitialized(&result, a.size() + b.size());
...
string StrCat(const AlphaNum &a, const AlphaNum &b, const AlphaNum &c) {
  string result;
  base::STLStringResizeUninitialized(&result, a.size() + b.size() + c.size());
  char *const begin = &*result.begin();
  char *out = Append2(begin, a, b);
  out = Append1(out, c);
  DCHECK_EQ(out, begin + result.size());
...
string StrCat(const AlphaNum &a, const AlphaNum &b, const AlphaNum &c,
              const AlphaNum &d) {
  string result;
  base::STLStringResizeUninitialized(&result,
...
string CatPieces(std::initializer_list<StringPiece> pieces) {
  ...
  base::STLStringResizeUninitialized(&result, total_size);
...
void StrAppend(string *result, const AlphaNum &a, const AlphaNum &b) {
  ...
  base::STLStringResizeUninitialized(result, old_size + a.size() + b.size());
  char *const begin = &*result->begin();
